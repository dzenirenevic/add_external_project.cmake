...
# Fetches the dependency using FetchContent from a git repository and optionally calls
# add_subdirectory if the external project root contains a CMakeLists.txt file.
# Arguments to the subdirectories' options can be passed as a list in ARGS and policies can be set
# using POLICIES.
function(add_external_project proj_name)
    ...
    if (${lc_name}_POPULATED)
        ...
        if (EXISTS ${${proj_name}_SOURCE_DIR}/CMakeLists.txt AND NOT ${ARG_SKIP_ADD_SUBDIR})
            foreach (arg ${ARG_ARGS})
                STRING(REGEX MATCH "^\(.*\)=\(.*)$" out ${arg})
                set(${CMAKE_MATCH_1} ${CMAKE_MATCH_2})
            endforeach ()

            foreach (policy ${ARG_POLICIES})
                STRING(REGEX MATCH "^\(.*\)=\(.*)$" out ${policy})
                set(CMAKE_POLICY_DEFAULT_${CMAKE_MATCH_1} ${CMAKE_MATCH_2})
            endforeach ()
